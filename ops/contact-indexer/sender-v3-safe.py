...
SEND_WORKERS=max(1,min(int(os.getenv('SEND_WORKERS','2')),4))
MAX_PER_HOUR=max(1,min(int(os.getenv('MAX_PER_HOUR','20')),20))

NAME='Guillaume'; COMPANY='EXIOR'; EMAIL='contact@exior.io'
...
NO_SOLICIT=('no solicitation','no solicitations','no unsolicited','do not use this form for sales','no sales enquiries','no sales inquiries','no vendors','no vendor solicitations','no marketing enquiries','no marketing inquiries')
...
def semantic(f):
    t=' '.join((f.get(k) or '') for k in ('name','id','placeholder','aria','label','nearby')).lower()
    typ=(f.get('type') or '').lower(); tag=(f.get('tag') or '').lower()
    ...
    async with pool.acquire() as c:
        async with c.transaction():
            ...
            row=await c.fetchrow('''
              SELECT q.company_id,q.form_id,q.form_url,co.domain
              ...
              WHERE q.status='MESSAGE_READY' AND f.status='CONTACTABLE' AND s.company_id IS NULL
              ORDER BY q.id
...
async def save(pool,row,status,confirmation,before,filled,after,payload):
    async with pool.acquire() as c:
        await c.execute('''INSERT INTO submissions_v3(company_id,form_id,form_url,status,confirmation,before_png,filled_png,after_png,result_json)
          ...
          ON CONFLICT(company_id) DO UPDATE SET status=EXCLUDED.status,confirmation=EXCLUDED.confirmation,before_png=EXCLUDED.before_png,filled_png=EXCLUDED.filled_png,after_png=EXCLUDED.after_png,result_json=EXCLUDED.result_json''',
...
async def worker(pool,browser,n):
    while True:
        ...
        try:
            ...
            await save(pool,row,status,confirmation,before,filled,after,payload); await ctx.close()
...
async def main():
    ...
    pool=await asyncpg.create_pool(PG_DSN,min_size=2,max_size=10); await init(pool)
