...
COMMERCIAL_OK=(
 'business inquiries','business enquiries','new business','partnership','partnerships',
 'vendor','vendors','supplier','suppliers','commercial inquiries','commercial enquiries',
 'general business','business contact'
)
NO_SOLICIT=(
...
async def worker(browser,n):
    while True:
        ...
        try:
            ...
            if not any(x in text for x in COMMERCIAL_OK):
                status='CHANNEL_NOT_EXPLICITLY_COMMERCIAL'
                raise RuntimeError('channel_not_commercial')
            ...
            if any(semantic(f)=='phone' and f['required'] for f in fields):
                status='REQUIRED_PHONE'; raise RuntimeError('phone')
            ...
            for f in fields:
                ...
                try:
                    if kind=='phone':
                        ...
                        await loc.fill(MESSAGE); message_field=True
                except Exception:
                    pass
            if not message_field:
                status='NO_MESSAGE_FIELD'; raise RuntimeError('no_message_field')

            await page.screenshot(path=str(filled),full_page=True)
            submit=page.locator("button[type=submit],input[type=submit]")
            if await submit.count()==0:
                status='NO_SUBMIT_BUTTON'; raise RuntimeError('no_submit')
            ...
            if any(x in result_html for x in SUCCESS_WORDS):
                status='SUCCESS'; confirmation='explicit_confirmation'
            elif new_url!=old_url and re.search(r'thank|success|submitted|confirmation',new_url,re.I):
                status='SUCCESS'; confirmation='confirmation_redirect'
            else:
            ...
            payload={'company_id':cid,'domain':domain,'form_url':url,'status':status,'confirmation':confirmation,'website_used':WEBSITE,'phone_used':False,'timestamp':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'screenshots':{'before':str(before),'filled':str(filled),'after':str(after)}}
