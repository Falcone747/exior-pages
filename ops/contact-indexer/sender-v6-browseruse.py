 ...
 async with pool.acquire() as c:
  async with c.transaction():
   row=await c.fetchrow('''
    ...
    WHERE q.status='DEFERRED_PRECHECK' AND a.company_id IS NULL
...
async def run_agent(url):
 ...
 try:
  ...
  text=''
  try: text=hist.final_result() or ''
  except Exception: text=str(hist)
  return text
 finally:
...
async def worker(pool,gate,n):
 while True:
  ...
  try:
   ...
   await save(pool,row,chosen,'SMART_ERROR','',{'error':f'{type(e).__name__}:{str(e)[:500]}','model':MODEL})
