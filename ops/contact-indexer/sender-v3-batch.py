...
class BatchGate:
    def __init__(self, limit:int):
        self.limit=limit; self.claimed=0; self.lock=asyncio.Lock()
...
async def worker(pool,browser,gate,n):
    while True:
        ...
        try:
            await page.goto(url,wait_until='domcontentloaded',timeout=30000); await page.wait_for_timeout(1400); await page.screenshot(path=str(before),full_page=True)
            ...
            old_url=page.url; await submit.first.click(timeout=10000); clicked=True; await page.wait_for_timeout(4500)
            ...
            payload={'company_id':cid,'domain':domain,'form_url':url,'status':status,'confirmation':confirmation,'clicked_submit':clicked,'website_used':WEBSITE,'phone_used':False,'batch_size':BATCH_SIZE,'timestamp':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'screenshots':{'before':str(before),'filled':str(filled),'after':str(after)}}
...
async def main():
    EVIDENCE.mkdir(parents=True,exist_ok=True); pool=await asyncpg.create_pool(PG_DSN,min_size=4,max_size=max(20,SEND_WORKERS+8)); await init(pool)
    gate=BatchGate(BATCH_SIZE)
    ...
    print('BATCH_DONE claimed=',gate.claimed,' ready_remaining=',ready,' results=',[(r['status'],r['n']) for r in rows],flush=True)
    await pool.close()
